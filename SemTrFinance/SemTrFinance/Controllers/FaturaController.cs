...
namespace SemTrFinance.Controllers
{
    ...
    {
        // GET: Cari
        [Menu("Fatura Listesi", "flaticon-users", "Fatura", 0,2)]
        public ActionResult Index()
        {
            return View();
        }

        [Yetki("Fatura Listesi Görüntüle", "Fatura")]
        public JsonResult Liste()
        {
            ...
            if (!string.IsNullOrEmpty(Request.Form["query[searchQuery]"]))
            {
                ...
                model = model.Where(i => i.BelgeNo.ToLower().Contains(searchQuery.ToLower()));
            }
            //Filtre
            ...
            var dto = model.Select(i => new
            {
                ...
                NormalIade= i.NormalIade.Adi,
                FaturaBelgeTuru=i.FaturaBelgeTuru.Adi,
                Cari=i.Cari.Unvan,
                BelgeNo=i.BelgeNo,
                //Tarih=i.BelgeTarih.Value.ToString("dd/MM/yyyy"),
                ProjeKodu=i.ProjeKodu,
                DovizCinsi=i.OrijinalDovizCisi,
                Tutar=i.HareketMeblagOrijinalDoviz,
