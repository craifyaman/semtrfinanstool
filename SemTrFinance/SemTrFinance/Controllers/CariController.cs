...
namespace SemTrFinance.Controllers
{
    ...
    {
        // GET: Cari
        [Menu("Cari Listesi", "flaticon-users", "Cari",0,2)]
        public ActionResult Index()
        {
            return View();
        }

        [Yetki("Cari Listesi Görüntüle", "Personel")]
        public JsonResult Liste()
        {
            ...
            if (!string.IsNullOrEmpty(Request.Form["query[searchQuery]"]))
            {
                ...
                model = model.Where(i => i.Unvan.ToLower().Contains(searchQuery.ToLower()));
            }
            //Filtre
