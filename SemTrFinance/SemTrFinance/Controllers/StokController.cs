...
namespace SemTrFinance.Controllers
{
    ...
    {
        // GET: Cari
        [Menu("Stok Listesi", "flaticon-users", "Stok",0,2)]
        public ActionResult Index()
        {
            return View();
        }

        [Yetki("Stok Listesi Görüntüle", "Stok")]
        public JsonResult Liste()
        {
            ...
            IQueryable<Stok> model = null;
            model = Db.Stok.AsQueryable();
            ...
            if (!string.IsNullOrEmpty(Request.Form["query[searchQuery]"]))
            {
                ...
                model = model.Where(i => i.StokAdi.ToLower().Contains(searchQuery.ToLower()));
            }
            //Filtre
            ...
            catch (Exception)
            {
                ...
                model = model.OrderBy("StokId Desc");
            }
            //sayfala
            ...
            var dto = model.Select(i => new
            {
                ...
                Kod = i.StokKodu,
                BirimAdi = i.BirimAdi,
                MuhasebeGrupKodu = i.StokMuhasebeGrupKodu
